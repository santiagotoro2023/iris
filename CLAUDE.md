...
- [~] Phase 2 — Voice I/O: **STT + TTS done**. `stt/` (faster-whisper large-v3) and `tts/` (XTTS v2, built-in speaker — voice source resolved, option b). Streaming text (`/chat/stream`, NDJSON) and per-sentence speech pipelining; both models idle-unload to share the GPU (SPEC.md §15–16). **Remaining: openWakeWord, turn-taking/barge-in**
