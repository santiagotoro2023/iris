...
- [~] Phase 3 — Memory System: **embedding, storage, recall and encrypted backups done** (`api/memory.py` bge-m3 + Qdrant, automatic recall into the system turn, Memory tab; `api/backup.py` + `restore.sh`, AES-256 archives in `./backup/`, gitignored — SPEC.md §25–26). Remaining: raw ingestion, diarization, nightly 30-day compaction
